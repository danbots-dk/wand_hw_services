...
class Eeprom():
    ...
    def set_version(self, val, idx):
        if len(val) > 10:
            print("String is too long, max 10 characters")
            return 0
        idx = self.eeprom_idx[idx]
        self.carrier_version = val
        self.write_bytes(val, idx)
        return 1

    def get_version(self, idx):
        return self.get_bytes(self.eeprom_idx[idx])

    def get_all_version(self):
        versions = ""
        for idx in self.eeprom_idx:
...
if __name__ == "__main__":
    ...
    #print(eeprom.get_version(self.eeprom_idx["img_install_date"]))
